...
class ThemePickerService:
        ...
        if isinstance(raw_stocks, list):
            ...
            if isinstance(selected_stock, dict) and selected_stock_code:
                for item in deduped_stocks:
                    ...
                    selected_changed = cls._hydrate_selected_stock_from_stock_item(selected_stock, item)
                    changed = changed or stock_changed or selected_changed
                    repaired = repaired or stock_changed or selected_changed
                    break
    ...
    @classmethod
    def _hydrate_selected_stock_from_stock_item(
        cls,
        selected_stock: Dict[str, Any],
        stock_item: Optional[Dict[str, Any]],
    ) -> bool:
        if not isinstance(selected_stock, dict) or not isinstance(stock_item, dict):
            return False

        changed = False
        field_pairs = [
            (("current_pattern", "currentPattern"), ("current_pattern", "currentPattern")),
            (("current_price", "currentPrice"), ("current_price", "currentPrice")),
            (("pct_chg", "pctChg"), ("pct_chg", "pctChg")),
            (("volume_ratio", "volumeRatio"), ("volume_ratio", "volumeRatio")),
            (("turnover_rate", "turnoverRate"), ("turnover_rate", "turnoverRate")),
            (("buy_signal", "buySignal"), ("buy_signal", "buySignal")),
            (("data_completeness", "dataCompleteness"), ("data_completeness", "dataCompleteness")),
        ...
        for target_keys, source_keys in field_pairs:
            ...
            for key in target_keys:
                if key in selected_stock:
                    current_key = key
                    current_value = selected_stock.get(key)
                    break
            ...
            for key in source_keys:
                if stock_item.get(key) not in (None, ""):
                    source_value = stock_item.get(key)
                    break
            ...
            selected_stock[target_key] = source_value
            changed = True

        existing_selected_reasons = selected_stock.get("selected_reasons", selected_stock.get("selectedReasons"))
        if not existing_selected_reasons:
            reasons = stock_item.get("mini_reasons") or stock_item.get("miniReasons") or []
            if not reasons:
                reason = stock_item.get("selection_reason") or stock_item.get("selectionReason")
                reasons = [reason] if reason else []
            if reasons:
                if "selected_reasons" in selected_stock:
                    selected_stock["selected_reasons"] = reasons
                elif "selectedReasons" in selected_stock:
                    selected_stock["selectedReasons"] = reasons
                else:
                    selected_stock["selected_reasons"] = reasons
                changed = True

        existing_risk_reasons = selected_stock.get("risk_reasons", selected_stock.get("riskReasons"))
        if not existing_risk_reasons:
            risk_note = stock_item.get("risk_note") or stock_item.get("riskNote")
            if risk_note:
                reasons = [risk_note]
                if "risk_reasons" in selected_stock:
                    selected_stock["risk_reasons"] = reasons
                elif "riskReasons" in selected_stock:
                    selected_stock["riskReasons"] = reasons
                else:
                    selected_stock["risk_reasons"] = reasons
                changed = True

        theme_relevance = selected_stock.get("theme_relevance", selected_stock.get("themeRelevance"))
        if theme_relevance in (None, ""):
            target_key = "theme_relevance" if "theme_relevance" in selected_stock or "themeRelevance" not in selected_stock else "themeRelevance"
            selected_stock[target_key] = "medium"
            changed = True

        trend_status = selected_stock.get("trend_status", selected_stock.get("trendStatus"))
        if trend_status in (None, ""):
            ma10 = cls._safe_float(selected_stock.get("ma10", selected_stock.get("ma10")))
            ma20 = cls._safe_float(selected_stock.get("ma20", selected_stock.get("ma20")))
            if ma10 is not None and ma20 is not None:
                derived = "多头排列" if ma10 >= ma20 else "震荡整理"
                target_key = "trend_status" if "trend_status" in selected_stock or "trendStatus" not in selected_stock else "trendStatus"
                selected_stock[target_key] = derived
                changed = True
