...
SETTING_DEFINITIONS: List[RuntimeSettingDefinition] = [
    RuntimeSettingDefinition(
        ...
        placeholder="./data/stock_analysis.duckdb",
    ...
    RuntimeSettingDefinition(
        ...
        label="实时行情优先级",
        input_type="text",
        description="逗号分隔；留空时走默认顺序，配置 Tushare token 后默认把 tushare 前置。",
        placeholder="tushare,tencent,akshare_sina,efinance,akshare_em",
    ),
    RuntimeSettingDefinition(
        ...
        label="主题行情优先级",
        input_type="text",
        description="主题选股使用的实时行情源优先级；留空时走默认顺序，配置 Tushare token 后默认把 tushare 前置。",
        placeholder="tushare,tencent,akshare_sina,efinance,akshare_em",
    ),
    RuntimeSettingDefinition(
        key="DAILY_BAR_SOURCE_PRIORITY",
        section_id="runtime",
        label="日K优先级",
        input_type="text",
        description="统一日K在线补数源；留空时走默认顺序，配置 Tushare token 后默认把 tushare 前置。",
        placeholder="tushare,mootdx",
    ),
    RuntimeSettingDefinition(
        key="CHIP_DISTRIBUTION_SOURCE_PRIORITY",
        section_id="runtime",
        label="筹码分布优先级",
        input_type="text",
        description="筹码分布数据源；留空时走默认顺序，配置 Tushare token 后默认把 tushare 前置。",
        placeholder="tushare,akshare",
    ),
    RuntimeSettingDefinition(
    ...
    RuntimeSettingDefinition(
        key="ETF_REALTIME_QUOTE_TIMEOUT_SECONDS",
        section_id="stock-query",
        label="ETF 实时行情超时（秒）",
        input_type="number",
        placeholder="8",
